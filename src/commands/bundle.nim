...
proc bundleAssetsCommand*(v: Values) =
  ...
  let skipPrefixKey = v.has("--skip-prefix")
  ...
  for f in files:
    let relToSrc = relativePath(f, srcDir).replace("\\", "/")
    let key =
      if not skipPrefixKey:
        "/" & (relToSrc).replace("\\", "/")    # "assets/app.css"
      else:
        (relToSrc).replace("\\", "/")          # "app.css"
    let raw = readFile(f)
    entries.add(
        Entry(key: key, off: off, size: raw.len,
              isText: isPlainText(raw)))
    blob.add(raw)
    off += raw.len
  ...
  lines.add("import pkg/supranim/service/assets")
  ...
  for e in entries:
    lines.add("  (" &
      ...
      (
        if not skipPrefixKey: nimStringLiteral("/" & keyPrefix / e.key) & ", "
        else: nimStringLiteral(e.key) & ", "
      ) &
      $e.off & ", " &
      $e.size & ", " &
